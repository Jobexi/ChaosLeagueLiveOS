...
public class GoldShop : Game
{
    [SerializeField] private List<GoldShopEntry> _entries;
        ...
        foreach (var entry in _entries)
        {
            foreach (string buyCommand in entry.BuyCommands)
            {
                if (msg.ToLower().StartsWith(buyCommand))
                {
                    ...
                    {
                        twitchClient.ReplyToPlayer(messageId, ph.pp.TwitchUsername, $"You don't have enough rubies! Your current rubies: {ph.pp.Rubies}");
                        return;
                    }
